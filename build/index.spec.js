...
const makeRoute = (option = {}) => {
    return Object.assign({ method: 'GET', path: '/', handler() {
            return 'foo';
        } }, option);
};
...
test('When request is made it honors */* accept header', async () => {
    ...
    expect(anyResp.headers['content-type']).toEqual('application/json; charset=utf-8');
...
test('When request is made it honors application/json accept header', async () => {
    ...
    expect(jsonPreferred.headers['content-type']).toEqual('application/json; charset=utf-8');
    expect(Object.keys(jsonPreferred.headers).includes('location')).toBeFalsy();
    expect(JSON.parse(jsonPreferred.payload).message).toEqual('Missing authentication');
});
test('When request is made it honors text/html accept header', async () => {
    ...
    expect(htmlResp.headers.location).toEqual('/login?next=' + encodeURIComponent('/'));
...
test('When GET /login request is made the response is correct', async () => {
    ...
    expect(response.headers['set-cookie'][0]).toEqual(expect.stringContaining('; Max-Age=86400; Expires='));
    expect(response.headers['set-cookie'][0]).toMatch(/; HttpOnly; SameSite=Strict; Path=\/$/);
    expect(response.headers.location).toEqual(expect.stringContaining('https://my-app.auth0.com/authorize?client_id=someclientid&response_type=code&redirect_uri=http%3A%2F%2F'));
    expect(response.headers.location).toEqual(expect.stringContaining('%2Flogin&state='));
...
test('When GET /logout request is made the respones is correct', async () => {
    ...
    expect(response.statusCode).toEqual(302);
    expect(response.statusMessage).toEqual('Found');
    expect(response.headers['set-cookie'][0]).toEqual('sid=; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; SameSite=Lax');
    expect(response.headers.location).toEqual(`https://my-app.auth0.com/v2/logout?returnTo=${encodeURIComponent('https://' + server.info.host + ':' + server.info.port + '/')}`);
    expect(response.payload).toEqual('');
});
test('When GET /logout redirects it goes to next=bah', async () => {
    ...
    expect(bare.headers.location).toEqual(`https://my-app.auth0.com/v2/logout?returnTo=${encodeURIComponent('https://' + server.info.host + ':' + server.info.port + '/bah')}`);
...
test('When GET /logout redirects it goes to next=/bah', async () => {
    ...
    expect(slash.headers.location).toEqual(`https://my-app.auth0.com/v2/logout?returnTo=${encodeURIComponent('https://' + server.info.host + ':' + server.info.port + '/bah')}`);
...
test('When GET /logout redirects it goes to next even if encoded', async () => {
    ...
    expect(encodedSlash.headers.location).toEqual(`https://my-app.auth0.com/v2/logout?returnTo=${encodeURIComponent('https://' + server.info.host + ':' + server.info.port + '/bah')}`);
...
test('When GET /logout rejects absolute urls for next', async () => {
    ...
    expect(absolute.headers['content-type']).toEqual('application/json; charset=utf-8');
    expect(JSON.parse(absolute.payload).message).toEqual('Absolute URLs are not allowed in the `next` parameter for security reasons');
});
test('When GET /logout rejects absolute urls for next encoded absolute', async () => {
    ...
    expect(encodedAbsolute.headers['content-type']).toEqual('application/json; charset=utf-8');
    expect(JSON.parse(encodedAbsolute.payload).message).toEqual('Absolute URLs are not allowed in the `next` parameter for security reasons');
});
test('When GET /logout rejects absolute urls for next schemeless', async () => {
    ...
    expect(schemeless.headers['content-type']).toEqual('application/json; charset=utf-8');
    expect(JSON.parse(schemeless.payload).message).toEqual('Absolute URLs are not allowed in the `next` parameter for security reasons');
});
test('When GET /logout rejects absolute urls for next like encoded schemeless', async () => {
    ...
    expect(encodedSchemeless.headers['content-type']).toEqual('application/json; charset=utf-8');
    expect(JSON.parse(encodedSchemeless.payload).message).toEqual('Absolute URLs are not allowed in the `next` parameter for security reasons');
});
